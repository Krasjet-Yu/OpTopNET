...
def reindex_dataset(csv_file_name):
    ...
    for row_index in range(0, datasz, 1):
        for item in range(20, 30, 1):
            df.iat[row_index, item] = change(int(df.iat[row_index, item]))

    df.to_csv(f"cycle_Topo_dataset_{datasz}_new.csv", sep=' ', index=False)

reindex_dataset(f"cycle_Topo_dataset_{datasz}.csv")

df = pd.read_csv(f"cycle_Topo_dataset_{datasz}_new.csv", delimiter=" ")
